...
There are `r nrow(activity)` observations and `r ncol(activity)` variables in the dataset. We transform the variable date in Date format in order to use it for parts of the following analyses:
...
The mean number of steps per day decreases from `r round(mean(stepDay$n_steps),2)` to `r round(mean(stepDaycomplete$n_steps),2)` while the median number of steps remains the same.
